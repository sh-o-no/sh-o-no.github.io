...
// デフォルトの画像を作成
let mainImage = document.createElement('img'); //変数 mainImage を定義 createElement(ﾒｿｯﾄﾞ)を使いimg要素を生成
// アルバムデータの先頭のものを使用
...
// 写真のメッセージ表示用の要素を作成
let mainMsg = document.createElement('p'); // 変数mainMsg を定義 createElement(ﾒｿｯﾄﾞ)を使いp要素を生成
...
for (let i = 0; i < album.length; i++) { // 繰り返し処理 iの値がalbumの長さになるまで
  let thumbImage = document.createElement('img'); //変数 thumbImg を定義 img要素を生成
  ...
  thumbFlame.insertBefore(thumbImage, null); // 取得した要素(thumbFlame)のnullの前(実際は一番後ろ)にthumbImgを挿入
}
